...
<div class="grid w-full place-items-center">
  <div class="grid w-full grid-cols-4 gap-2 rounded-xl bg-gray-200 p-2">
    ...
    <div v-for="(v, k) in columns">
      <input v-model="model" :type="type" :id="name + '_' + k" :value="k" class="peer hidden"/>
      <label :for="name + '_' + k" class="text-black block cursor-pointer select-none rounded-xl p-2 text-center peer-checked:bg-blue-500 peer-checked:font-bold peer-checked:text-white">{{v}}</label>
    </div>
  </div>
</div>
</template>
